...
本 skill 不调任何外部 API、不额外付费。所有 LLM 判断由当前运行 skill 的 Agent（本 Claude 会话）直接完成。

### 2.1 粗剪流程（Step by Step）
...
python3 src/pipeline.py transcribe \
    ...
    --outdir output/本期/
# → output/本期/transcript.json（词级时间戳）
```

**Step 2：Agent 阅读转写 + 定稿，标注 NG 窗口**

```bash
# 输出分析 prompt（Agent 在本对话中阅读）
python3 src/ng_detect.py prompt \
    --transcript output/本期/transcript.json \
    --script     materials/scripts/定稿.md
```

Agent 阅读上述输出后，在对话中给出 NG 窗口 JSON，写入文件：

```json
// output/本期/ng_windows.json
[
...
**Step 3：重建 EDL + 渲染 + 滤镜 + QC**

```bash
python3 src/pipeline.py run \
    --media  reference/原素材.MP4 \
    --outdir output/本期/ \
    --ng-json output/本期/ng_windows.json
# → output/本期/roughcut_hd.mp4 + edl_snapped.json + qc_report.md
```

**断点续跑**：中途中断后重新运行同命令，已完成步骤自动跳过。
...
### Step ⑥ 字幕（可选，Agent 主导翻译）

```bash
# Step 6a：生成中文字幕 JSON（含时间轴，供 Agent 翻译）
python3 src/subtitle.py align \
    --transcript output/本期/transcript.json \
    --script     materials/scripts/定稿.md \
    --edl        output/本期/edl_snapped.json \
    --out        output/本期/subtitle_cn.json
```

Agent 阅读 `subtitle_cn.json`，逐句翻译后写出 `subtitle_en.json`：

```json
// output/本期/subtitle_en.json
[
  {"index": 1, "en": "Today we're talking about SRN901..."},
  {"index": 2, "en": "This compound extends median lifespan by 33%"}
]
```

或 Agent 直接写双语 JSON（快捷路径）：

```json
// output/本期/subtitle_bilingual.json
[
  {"cn": "今天介绍SRN901", "en": "Today: SRN901", "start_s": 1.2, "end_s": 3.5},
  ...
]
```

```bash
# Step 6b：合并生成双语 SRT + ASS
python3 src/subtitle.py generate \
    --cn-json output/本期/subtitle_cn.json \
    --en-json output/本期/subtitle_en.json \
    --out     output/本期/subtitle.srt \
    --ass

# 或快捷路径（双语 JSON）
python3 src/subtitle.py generate \
    --bilingual output/本期/subtitle_bilingual.json \
    --out       output/本期/subtitle.srt \
    --ass
